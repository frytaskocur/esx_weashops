		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, 38) then
				...
				if CurrentAction == 'shop_menu' then
					ESX.TriggerServerCallback('gene:checkLicense', function(data)
						local dzban = data
						if dzban == true or Config.Zones[CurrentActionData.zone].legal == 1 then
							OpenShopMenu(CurrentActionData.zone)
						else
							ESX.ShowNotification('~w~Nie posiadasz ~p~licencji ~r~(~w~Możesz ~p~wyrobić ~w~ją na ~b~Policji~r~)')
						end
...
Citizen.CreateThread(function()
    ...
    while not HasModelLoaded(GetHashKey("s_m_y_dealer_01")) do
      ...
      local ped =  CreatePed(4, 0xE497BBEF, 784.07, 4179.18, 40.89, 170.89, false, true)
